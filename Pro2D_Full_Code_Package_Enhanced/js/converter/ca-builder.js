  ...
  try {
    ...
    if (primary && primary.raw) {
      ...
      if (rowData) {
        ...
        // Piping Class -> PIPING-SPEC
        if (sm.PipingClass && rowData[sm.PipingClass]) {
            const pcVal = String(rowData[sm.PipingClass]).trim();
            if (pcVal) {
                lines.push(`${INDENT}PIPING-SPEC  ${pcVal}`);
                writtenAttrs.add('PIPING-SPEC');
            }
        }
