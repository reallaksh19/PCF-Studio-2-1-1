...
const BLACKLIST_PATTERNS = [
    ...
    /^\s{4}PIPELINE-REFERENCE\s+.*_Sp[0-9]+/i,        // Split tracking tag
    /^\s{4}COMPONENT-ATTRIBUTE9[78]\s+=\d+\/\d+_Injected/i,
    /^\s{4}COMPONENT-ATTRIBUTE9[78]\s+=\d+\/\d+_Sp[0-9]+/i,
...
export function filterPcfLines(lines) {
    ...
    return lines.filter(line => {
        ...
        if (match) {
            ...
            const UNDEFINED_PLACEHOLDERS = new Set(['UNDEFINED', 'UNDEFINED MM', 'UNDEFINED KG', 'UNDEFINED KPA']);
            if (UNDEFINED_PLACEHOLDERS.has(value.toUpperCase())) {
                return false;
            }
