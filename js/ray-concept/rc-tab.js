...
// ── Bootstrap (called from app.js) ───────────────────────────────────────────
  ...
  // Listen for the push back from PCF Fixer
  window.addEventListener('pcf-fixer-push-to-2d-csv', (e) => {
      const pushedData = e.detail;
      if (!pushedData || !Array.isArray(pushedData)) return;

      const cfg = getRayConfig();
      rcState.finalComponents = pushedData;
      rcState.finalCsv2DText = emit2DCSV(pushedData, cfg);

      const root = document.getElementById('ray-concept-tab-root') || document;

      render2DTable(root, rcState.finalCsv2DText, rcState.finalComponents);
      activatePreviewBtn(root, 'final2dcsv');
      switchPreview(root, 'final2dcsv');
      passLog(root, `✓ Received ${pushedData.length} rows from PCF Fixer`, 'success');
  });
