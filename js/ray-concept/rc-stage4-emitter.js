...
function emitPipe(ep1, ep2, bore, pipelineRef, seqNo, cfg, refNo, comp = {}) {
  ...
  lines.push(...emitCABlock(buildCompCa(comp), 'PIPE', refNo || comp.ca97 || comp.refNo || null, seqNo));
  lines.push('');
  return lines;
}
...
function emitFlange(comp, seqNo, cfg) {
  ...
  lines.push(...emitCABlock(buildCompCa(comp), 'FLANGE', comp.ca97 || comp.refNo || null, seqNo));
  lines.push('');
  return lines;
}
...
function emitBend(comp, seqNo, cfg) {
  ...
  lines.push(...emitCABlock(buildCompCa(comp), 'BEND', comp.ca97 || comp.refNo || null, seqNo));
  lines.push('');
  return lines;
}
...
function emitTee(comp, seqNo, cfg) {
  ...
  lines.push(`${pad(4)}<SKEY> ${sanitizeAttrValue(comp.skey)}`);
  lines.push(...emitCABlock(buildCompCa(comp), 'TEE', comp.ca97 || comp.refNo || null, seqNo));
  lines.push('');
  return lines;
}
...
function emitOlet(comp, seqNo, cfg) {
  ...
  lines.push(`${pad(4)}<SKEY> ${sanitizeAttrValue(comp.skey)}`);
  lines.push(...emitCABlock(buildCompCa(comp), 'OLET', comp.ca97 || comp.refNo || null, seqNo));
  lines.push('');
  return lines;
}
...
function emitValve(comp, seqNo, cfg) {
  ...
  lines.push(`${pad(4)}END-POINT    ${fmtCoord(comp.ep2, comp.bore, cfg)}`);
  lines.push(`${pad(4)}<SKEY> ${sanitizeAttrValue(comp.skey)}`);
  lines.push(...emitCABlock(buildCompCa(comp), 'VALVE', comp.ca97 || comp.refNo || null, seqNo));
  lines.push('');
  return lines;
}
...
function emitReducer(comp, seqNo, cfg) {
  ...
  lines.push(`${pad(4)}END-POINT    ${fmtCoord(comp.ep1, comp.bore, cfg)}`);
  lines.push(`${pad(4)}END-POINT    ${fmtCoord(comp.ep2, comp.bore, cfg)}`);
  lines.push(`${pad(4)}<SKEY> ${sanitizeAttrValue(comp.skey || 'RCBW')}`);
  lines.push(...emitCABlock(buildCompCa(comp), 'REDUCER-CONCENTRIC', comp.ca97 || comp.refNo || null, seqNo));
  lines.push('');
  return lines;
}
...
function emitBlindFlange(comp, seqNo, cfg) {
  ...
  lines.push(`${pad(4)}END-POINT    ${fmtCoord(comp.ep1, comp.bore, cfg)}`);
  lines.push(`${pad(4)}END-POINT    ${fmtCoord(comp.ep2, comp.bore, cfg)}`);
  lines.push(`${pad(4)}<SKEY> ${sanitizeAttrValue(comp.skey || 'BLFL')}`);
  lines.push(...emitCABlock(buildCompCa(comp), 'FLANGE', comp.ca97 || comp.refNo || null, seqNo));
  lines.push('');
  return lines;
}
...
export function runStage4(components, injectedPipes, pipelineRef, logFn = () => {}) {
  ...
  function emitBridgeSplit(br, fromRefNo) {
    ...
    if (hits.length === 0) {
      ...
      outputLines.push(...emitPipe(br.ep1, br.ep2, br.bore, pipelineRef, seqCtr, cfg, ep1RefNo, br));
      return;
    }
    ...
    for (const hit of hits) {
      ...
      outputLines.push(...emitPipe(cursor, hit.snap, br.bore, pipelineRef, seqCtr, cfg, segRefNo, br));
    ...
    outputLines.push(...emitPipe(cursor, br.ep2, br.bore, pipelineRef, seqCtr, cfg, ep1RefNo, br));
  }
