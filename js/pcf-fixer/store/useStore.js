...
export const useStore = create((set, get) => ({
  ...
  canvasMode: 'VIEW', // 'VIEW' | 'CONNECT' | 'STRETCH' | 'BREAK' | 'INSERT_SUPPORT' | 'MEASURE' | 'MARQUEE_SELECT' | 'MARQUEE_ZOOM' | 'MARQUEE_DELETE' | 'INSERT_VALVE'
  setCanvasMode: (mode) => {
  ...
  setExternalDataTable: (rows) => {
      const components = rows.map((row, idx) => {
          const ca = {};
          for (let n = 1; n <= 10; n++) {
              ca[n] = row.ca?.[n] ?? row[`CA${n}`] ?? row[`ca${n}`] ?? '';
          }
          ...
          return {
              ...
              rating: row.rating ?? '',
              PIPING_CLASS: row.PIPING_CLASS || row.pipingClass || '',
              RATING: row.RATING || row.rating || '',
              LINENO_KEY: row.LINENO_KEY || row.lineNoKey || '',
              CA1: ca[1] || '', CA2: ca[2] || '', CA3: ca[3] || '', CA4: ca[4] || '', CA5: ca[5] || '',
              CA6: ca[6] || '', CA7: ca[7] || '', CA8: ca[8] || '', CA9: ca[9] || '', CA10: ca[10] || '',
              ca1: ca[1] || '', ca2: ca[2] || '', ca3: ca[3] || '', ca4: ca[4] || '', ca5: ca[5] || '',
              ca6: ca[6] || '', ca7: ca[7] || '', ca8: ca[8] || '', ca9: ca[9] || '', ca10: ca[10] || '',
              ca97: row.ca97 || row.CA97 || '',
              ca98: row.ca98 || row.CA98 || '',
              ca,
              points: points,
              ep1: row.ep1,
              ep2: row.ep2,
